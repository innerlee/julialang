...
虽然目前还没有一个机器学习专属的语言，但实事求是地说，已有几种新语言（像 TensorFlow）藏身于 Python 语言接口之后；而另一些（比如 PyTorch）则直接用 Python 作为自己的建模语言。我们问这样一个问题——需不需要一个专门为机器学习定制的全新的语言？如果是，为什么？更重要地，如果将来有一个理想的机器学习语言，它会长什么样？
...
这种方式，至少以现在的样子，有一个客观的不利因素，就是需要引入元编程。构建和计算表达式树给程序员和编译器同时施加了极大的额外负担。现在代码有两个运行时间，每个都有着不同的语言语义，单步调试这样的操作也极其困难，这些都让程序变得难以理解。给新运行时创造一个句法语言可以解决这个问题，但这又何异于创造一个新的完整的编程语言！在已经有非常流行的数值语言的情况下，这么做还有意义吗？
...
没有几个领域像机器学习这样对语言层面的设计有如此苛刻的要求。当然不是说这没有先例，比如[形式推理与验证](https://coq.inria.fr/)和[集群计算](https://chapel-lang.org/)中的经验就证实了，新的定制的语言是非常有效的解决方案。同样我们希望看到，不管是创造新的还是利用现有的，为机器学习所需的数值、微分、并行及概率计算等功能而定制的语言。
...
*可编程语义*开启了灵活性的新层次，而且可以用类似于宏这样的功能来实现。这将的话，只要在核心系统上指定哪些代码应该有纯数据流语义，就可以实现多 GPU 训练这样的功能（与此对照，标准的祈使语义更灵活，但有不能安全优化的副作用）。它也可以实现概率编程语言所需的操作，以及 NLP 模型中通常手动实现的[向量化处理](https://www.cs.cmu.edu/~guyb/papers/Ble90.pdf)（批处理）。
...
机器学模型已是极其通用的信息处理系统，它们创建了从未有过的高层的，更复杂的抽象；通过基本组件组合，可以实现循环，递归，高阶模型，甚至[堆栈机](https://nlp.stanford.edu/blog/hybrid-tree-sequence-neural-networks-with-spinn/)和[语言解释器](https://arxiv.org/abs/1605.06640)。机器学习是一个新的编程范式，即便它满身的数值、微分、和并行标签让人觉得有些奇怪。像任何工程领域一样，手中的工具对将来工作的范围和质量有着不可估量的影响。
